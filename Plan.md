...
Retrospective outcome-classification rule-set version 1 uses per-arm medians, prespecified engineering change thresholds, direction consistency, minimum evidence gates, explicit confounder uncertainty, and adverse-effect precedence to produce exactly the six classifications and four analytical next actions in this plan. The thresholds are not clinical cutoffs and the result is not causal. `docs/decisions/0006-outcome-classification-rules.md` governs the complete evidence, boundary, precedence, and action contract.
