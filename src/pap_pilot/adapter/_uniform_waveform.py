"""Shared decoding for explicitly configured OSCAR uniform waveforms."""
...
_UNIFORM_WAVEFORM_EVENT_TYPE: Final = 0


class UniformWaveformAvailability(StrEnum):
    """Why an explicitly selected waveform is or is not available."""
...
class UniformWaveformSpec:
    """The allowlisted schema contract for one uniform waveform."""
...
@dataclass(frozen=True, slots=True)
class DecodedUniformWaveformSegment:
    """One validated, decoded EventList before public signal mapping."""
...
@dataclass(frozen=True, slots=True)
class DecodedUniformWaveform:
    """Validated source data for one allowlisted session waveform."""

    session_summary: OscarSessionSummary
    availability: UniformWaveformAvailability
    source_channel_id: int | None
    channel_code: str
    canonical_unit: str
    segments: tuple[DecodedUniformWaveformSegment, ...]
    source_class: OscarSignalSourceClass


def extract_uniform_waveform(
    ...
    *,
    ...
    """Extract one configured waveform for exactly one validated session."""
...
def _extract_uniform_waveform(
    ...
    *,
    ...
    return DecodedUniformWaveform(
        ...
        availability=UniformWaveformAvailability.AVAILABLE,
        source_channel_id=channel_id,
    ...
    *,
    ...
    rows = connection.execute(
        ...
        WHERE profile_id = ?
          AND channel_code = ?
        """,
        (session_summary.profile.database_id, spec.channel_code),
    ).fetchall()
    if not rows:
        return None
    if len(rows) != 1:
        raise error_type(
            f"The profile has ambiguous {spec.signal_name} channel provenance."
        )
    return _required_integer(
        rows[0]["channel_id"],
        f"{spec.signal_name} channel identifier",
        error_type=error_type,
    )
...
def _segments_from_rows(
    ...
    *,
    spec: UniformWaveformSpec,
    error_type: type[OscarDatabaseError],
) -> tuple[DecodedUniformWaveformSegment, ...]:
    segments: list[DecodedUniformWaveformSegment] = []
    previous_end_ms: int | None = None
    for expected_index, row in enumerate(rows):
        eventlist_index = _required_integer(
            row["eventlist_index"],
            f"{spec.signal_name} EventList index",
            error_type=error_type,
        )
        if eventlist_index != expected_index:
            raise error_type(
                f"{spec.signal_name} EventList indexes are not zero-based "
                "and contiguous."
            )
...
def _segment_from_row(
    ...
    *,
    ...
    if _required_integer(
        row["event_type"],
        f"{spec.signal_name} event type",
        error_type=error_type,
    ) != _UNIFORM_WAVEFORM_EVENT_TYPE:
        raise error_type(
            f"{spec.signal_name} is not stored as a uniform waveform EventList."
        )
    if _required_text(
        row["source_dimension"],
        f"{spec.signal_name} dimension",
        error_type=error_type,
    ) != spec.source_dimension:
        raise error_type(
            f"{spec.signal_name} does not have the expected OSCAR source "
            "dimension."
        )
    if _required_boolean(
        row["has_second_field"],
        f"{spec.signal_name} has_second_field",
        error_type=error_type,
    ):
        raise error_type(
            f"{spec.signal_name} unexpectedly contains a second value field."
        )
    if any(
        ...
        raise error_type(
            f"{spec.signal_name} contains unsupported secondary or "
            "explicit-time data."
        )

    sample_count = _required_integer(
        row["sample_count"],
        f"{spec.signal_name} sample count",
        error_type=error_type,
    )
    if sample_count <= 0:
        raise error_type(
            f"{spec.signal_name} has a non-positive sample count."
        )
    sample_interval_ms = _required_number(
        row["sample_interval_ms"],
        f"{spec.signal_name} sample interval",
        error_type=error_type,
    )
    if not math.isclose(
        sample_interval_ms,
        ...
        raise error_type(
            f"{spec.signal_name} does not have the required 25 Hz sample "
            "interval."
        )

    raw_first_time_ms = _required_integer(
        row["raw_first_time_ms"],
        f"{spec.signal_name} first timestamp",
        error_type=error_type,
    )
    raw_end_time_ms = _required_integer(
        row["raw_end_time_ms_exclusive"],
        f"{spec.signal_name} end timestamp",
        error_type=error_type,
    )
    ...
    if not math.isclose(
        ...
        raise error_type(
            f"{spec.signal_name} bounds do not match its count and sample "
            "interval."
        )
    ...
    if (
        ...
        raise error_type(
            f"{spec.signal_name} boundaries fall outside the selected session."
        )
    ...
    if previous_end_ms is not None:
        ...
        if gap_before_ms < 0:
            raise error_type(
                f"{spec.signal_name} EventLists overlap or are not "
                "chronological."
            )

    gain = _required_number(
        row["gain"],
        f"{spec.signal_name} gain",
        error_type=error_type,
    )
    offset = _required_number(
        row["offset"],
        f"{spec.signal_name} offset",
        error_type=error_type,
    )
    if gain <= 0:
        raise error_type(f"{spec.signal_name} has a non-positive gain.")
    data_size_bytes = _required_integer(
        row["data_size_bytes"],
        f"{spec.signal_name} uncompressed data size",
        error_type=error_type,
    )
    expected_data_size = sample_count * 2
    if data_size_bytes != expected_data_size:
        raise error_type(
            f"{spec.signal_name} data_size does not match its sample count."
        )
    ...
    source_checksum = _required_integer(
        row["source_checksum"],
        f"{spec.signal_name} checksum",
        error_type=error_type,
    )
    if not 0 <= source_checksum <= 0xFFFF:
        raise error_type(
            f"{spec.signal_name} checksum is outside uint16 range."
        )
    if _qt_iso3309_checksum(primary_data) != source_checksum:
        raise error_type(f"{spec.signal_name} checksum validation failed.")
    ...
    if any(not math.isfinite(value) for value in values):
        raise error_type(f"{spec.signal_name} produced a non-finite value.")
    sample_times = tuple(
        raw_first_time_ms + index * sample_interval_ms
        for index in range(sample_count)
    )

    return DecodedUniformWaveformSegment(
        source_eventlist_id=_required_integer(
            row["source_eventlist_id"],
            f"{spec.signal_name} EventList identifier",
            error_type=error_type,
        ),
        source_event_data_id=_required_integer(
            row["source_event_data_id"],
            f"{spec.signal_name} data-row identifier",
            error_type=error_type,
        ),
...
def _validate_segment_provenance(
    ...
    *,
    ...
    for field, expected, label in expected_values:
        if _required_integer(
            row[field],
            f"{spec.signal_name} {label} identifier",
            error_type=error_type,
        ) != expected:
            raise error_type(
                f"A {spec.signal_name} EventList has conflicting {label} "
                "provenance."
            )
...
def _decode_primary_data(
    ...
    *,
    ...
    compression_method = _required_integer(
        ...
        f"{spec.signal_name} compression method",
        error_type=error_type,
    )
    ...
    if compression_method == 0:
        if not isinstance(raw_blob, bytes) or compressed_blob is not None:
            raise error_type(
                f"{spec.signal_name} has invalid uncompressed primary storage."
            )
        ...
        if raw_blob is not None or not isinstance(compressed_blob, bytes):
            raise error_type(
                f"{spec.signal_name} has invalid compressed primary storage."
            )
        ...
        raise error_type(
            f"{spec.signal_name} uses an unsupported compression method."
        )

    if len(primary_data) != expected_size:
        raise error_type(
            f"{spec.signal_name} primary data length does not match its "
            "sample count."
        )
    compressed_size = _optional_integer(
        row["compressed_size_bytes"],
        f"{spec.signal_name} stored data size",
        error_type=error_type,
    )
    if compressed_size is not None and compressed_size != stored_size:
        raise error_type(
            f"{spec.signal_name} compressed_size does not match the selected "
            "storage BLOB."
        )
...
def _qt_uncompress(
    ...
    *,
    ...
    if len(data) < 5:
        raise error_type(f"{spec.signal_name} qCompress data is truncated.")
    header_size = int.from_bytes(data[:4], byteorder="big", signed=False)
    if header_size != expected_size:
        raise error_type(
            f"{spec.signal_name} qCompress length header does not match "
            "data_size."
        )
    ...
    try:
        ...
        raise error_type(
            f"{spec.signal_name} qCompress data could not be decompressed."
        ) from error
    if (
        ...
        raise error_type(
            f"{spec.signal_name} qCompress data has an incomplete or "
            "trailing stream."
        )
    return result
...
def _required_integer(
    ...
    *,
    ...
    if type(value) is not int:
        raise error_type(f"The OSCAR {field_name} is not an integer.")
    return value
...
def _optional_integer(
    ...
    *,
    ...
    if value is None:
        return None
    return _required_integer(value, field_name, error_type=error_type)
...
def _required_number(
    ...
    *,
    ...
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise error_type(f"The OSCAR {field_name} is not numeric.")
    number = float(value)
    if not math.isfinite(number):
        raise error_type(f"The OSCAR {field_name} is not finite.")
    return number
