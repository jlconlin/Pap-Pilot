"""Versioned experiment and append-only event records."""

from pap_pilot.engine.experiments.allocation import (
    EXPERIMENT_ALLOCATION_ENGINE_VERSION,
    EXPERIMENT_ALLOCATION_RULE_SET_ID,
    EXPERIMENT_ALLOCATION_RULE_SET_VERSION,
    AllocationInterval,
    ExperimentAllocationError,
    ExperimentNightAllocation,
    ExperimentPeriod,
    NightAllocation,
    NightAllocationStatus,
    allocate_experiment_nights,
)
from pap_pilot.engine.experiments.classification import (
    DIRECTION_CONSISTENCY_DENOMINATOR,
    DIRECTION_CONSISTENCY_NUMERATOR,
    MINIMUM_CLASSIFICATION_NIGHTS_PER_ARM,
    OUTCOME_CLASSIFICATION_ENGINE_VERSION,
    OUTCOME_CLASSIFICATION_RECORD_VERSION,
    OUTCOME_CLASSIFICATION_RULE_SET_ID,
    OUTCOME_CLASSIFICATION_RULE_SET_VERSION,
    OUTCOME_THRESHOLDS,
    ArmConfounderSummary,
    ConfounderEvidence,
    ConfounderEvidenceStatus,
    NightlyOutcomeValue,
    OutcomeAction,
    OutcomeArmSummary,
    OutcomeClassification,
    OutcomeClassificationError,
    OutcomeClassificationResult,
    OutcomeDirection,
    OutcomeEvidence,
    OutcomeId,
    OutcomeState,
    OutcomeThreshold,
    SubjectiveDomainState,
    evaluate_outcome_classification,
)
from pap_pilot.engine.experiments.corrections import build_boundary_correction_events
from pap_pilot.engine.experiments.generic import (
    GENERIC_EXPERIMENT_ANALYSIS_ENGINE_VERSION,
    GENERIC_EXPERIMENT_RECORD_VERSION,
    GENERIC_EXPERIMENT_SAFETY_DISPATCH_VERSION,
    GENERIC_EXPERIMENT_SCHEMA_ID,
    GENERIC_EXPERIMENT_SCHEMA_VERSION,
    GenericExperimentAnalysis,
    GenericExperimentAnalysisStatus,
    GenericExperimentChange,
    GenericExperimentComparison,
    GenericExperimentDefinition,
    GenericExperimentError,
    GenericExperimentMetricObservation,
    GenericExperimentMetricSelection,
    GenericExperimentMetricStatus,
    GenericExperimentMode,
    GenericExperimentPeriod,
    GenericExperimentPeriodRole,
    GenericExperimentSafetyPolicy,
    GenericExperimentSafetyResult,
    GenericExperimentSafetyStatus,
    GenericExperimentVariableKind,
    PsMinSafetyPolicyInput,
    analyze_generic_experiment,
    dispatch_generic_experiment_safety,
)
from pap_pilot.engine.experiments.journal import (
    JOURNAL_RATING_MAXIMUM,
    JOURNAL_RATING_MINIMUM,
    RETROSPECTIVE_USER_EVIDENCE_RECORD_VERSION,
    RETROSPECTIVE_USER_EVIDENCE_SCHEMA_ID,
    RETROSPECTIVE_USER_EVIDENCE_SCHEMA_VERSION,
    SLEEP_JOURNAL_RECORD_VERSION,
    SLEEP_JOURNAL_SCHEMA_ID,
    SLEEP_JOURNAL_SCHEMA_VERSION,
    ConfounderKind,
    ConfounderReportStatus,
    RetrospectiveEvidenceStatus,
    RetrospectiveNightEvidence,
    RetrospectiveObservation,
    SleepJournalConfounder,
    SleepJournalEntry,
    SleepJournalModelError,
)
from pap_pilot.engine.experiments.model import (
    EXPERIMENT_EVENT_RECORD_VERSION,
    EXPERIMENT_EVENT_SCHEMA_ID,
    EXPERIMENT_EVENT_SCHEMA_VERSION,
    EXPERIMENT_RECORD_VERSION,
    EXPERIMENT_SCHEMA_ID,
    EXPERIMENT_SCHEMA_VERSION,
    EvaluationIssuedPayload,
    EvaluationSupersededPayload,
    AiProvenancePayload,
    ExperimentActionPayload,
    ExperimentDecisionPayload,
    ExperimentEvent,
    ExperimentEventPayload,
    ExperimentEventType,
    ExperimentEvidenceInterval,
    ExperimentModelError,
    ExperimentProposal,
    ExperimentProposedPayload,
    ExperimentRecord,
    ExperimentRevisedPayload,
    ExperimentSetting,
    ExperimentSettingChange,
    HypothesisDraftedPayload,
    NoteRecordedPayload,
    ObservationRecordedPayload,
    ProblemRecordedPayload,
    SettingChangeConfirmedPayload,
    SleepJournalEntryRecordedPayload,
    append_experiment_event,
    validate_experiment_history,
)
from pap_pilot.engine.experiments.retrospective_fixture import (
    PS_MIN_RETROSPECTIVE_FIXTURE_ENGINE_VERSION,
    PS_MIN_RETROSPECTIVE_FIXTURE_ID,
    PS_MIN_RETROSPECTIVE_FIXTURE_RECORD_VERSION,
    PS_MIN_RETROSPECTIVE_FIXTURE_VERSION,
    PSMinRetrospectiveFixture,
    RetrospectiveFixtureError,
    RetrospectiveFixtureEvaluation,
    RetrospectiveFixtureEvaluationStatus,
    RetrospectiveKnownFact,
    RetrospectiveMissingInput,
    RetrospectiveMissingInputId,
    reconstruct_ps_min_experiment_fixture,
)
from pap_pilot.engine.experiments.retrospective_evidence import (
    RETROSPECTIVE_USER_EVIDENCE_INTAKE_VERSION,
    RetrospectiveEvidenceBatch,
    RetrospectiveEvidenceError,
    RetrospectiveNightEvidenceInput,
    build_retrospective_user_evidence,
    record_retrospective_user_evidence,
)
from pap_pilot.engine.experiments.retrospective_evaluation import (
    RETROSPECTIVE_EVALUATION_ENGINE_VERSION,
    RETROSPECTIVE_EVALUATION_RECORD_VERSION,
    RETROSPECTIVE_EVALUATION_SCHEMA_ID,
    RETROSPECTIVE_EVALUATION_SCHEMA_VERSION,
    RetrospectiveEvaluationBundle,
    RetrospectiveEvaluationError,
    RetrospectiveSessionClockEvidence,
    evaluate_retrospective_experiment,
)
from pap_pilot.engine.experiments.retrospective_protocol import (
    RETROSPECTIVE_PROTOCOL_RECORD_VERSION,
    RetrospectiveCohortEvidence,
    RetrospectiveProtocolError,
    RetrospectiveProtocolInput,
    build_retrospective_protocol_events,
    record_retrospective_protocol,
)
from pap_pilot.engine.experiments.safety import (
    MINIMUM_PROSPECTIVE_NIGHTS_PER_ARM,
    MINIMUM_PROSPECTIVE_USABLE_DURATION_MS,
    PROSPECTIVE_SAFETY_ENGINE_VERSION,
    PROSPECTIVE_SAFETY_POLICY_ID,
    PROSPECTIVE_SAFETY_POLICY_VERSION,
    ProspectiveNightEvidence,
    ProspectiveSafetyEvidence,
    ProspectiveSafetyFailureCode,
    ProspectiveSafetyGateResult,
    evaluate_prospective_safety,
)
from pap_pilot.engine.experiments.lifecycle import (
    PROSPECTIVE_LIFECYCLE_ENGINE_VERSION,
    ProspectiveLifecycleError,
    ProspectiveLifecycleState,
    ProspectiveLifecycleStatus,
    append_prospective_lifecycle_event,
    replay_prospective_lifecycle,
)
from pap_pilot.engine.experiments.storage import (
    EXPERIMENT_STORE_RECORD_FORMAT,
    EXPERIMENT_STORE_RECORD_FORMAT_VERSION,
    EXPERIMENT_STORE_SCHEMA_ID,
    EXPERIMENT_STORE_SCHEMA_VERSION,
    LOCAL_EXPERIMENT_DATABASE_FILENAME,
    ExperimentNotFoundError,
    ExperimentStore,
    ExperimentStoreError,
    ReplayedExperiment,
)

__all__ = [
    "DIRECTION_CONSISTENCY_DENOMINATOR",
    "DIRECTION_CONSISTENCY_NUMERATOR",
    "EXPERIMENT_ALLOCATION_ENGINE_VERSION",
    "EXPERIMENT_ALLOCATION_RULE_SET_ID",
    "EXPERIMENT_ALLOCATION_RULE_SET_VERSION",
    "JOURNAL_RATING_MAXIMUM",
    "JOURNAL_RATING_MINIMUM",
    "RETROSPECTIVE_USER_EVIDENCE_INTAKE_VERSION",
    "RETROSPECTIVE_USER_EVIDENCE_RECORD_VERSION",
    "RETROSPECTIVE_USER_EVIDENCE_SCHEMA_ID",
    "RETROSPECTIVE_USER_EVIDENCE_SCHEMA_VERSION",
    "RETROSPECTIVE_EVALUATION_ENGINE_VERSION",
    "RETROSPECTIVE_EVALUATION_RECORD_VERSION",
    "RETROSPECTIVE_EVALUATION_SCHEMA_ID",
    "RETROSPECTIVE_EVALUATION_SCHEMA_VERSION",
    "SLEEP_JOURNAL_RECORD_VERSION",
    "SLEEP_JOURNAL_SCHEMA_ID",
    "SLEEP_JOURNAL_SCHEMA_VERSION",
    "EXPERIMENT_EVENT_RECORD_VERSION",
    "EXPERIMENT_EVENT_SCHEMA_ID",
    "EXPERIMENT_EVENT_SCHEMA_VERSION",
    "EXPERIMENT_RECORD_VERSION",
    "EXPERIMENT_SCHEMA_ID",
    "EXPERIMENT_SCHEMA_VERSION",
    "EXPERIMENT_STORE_RECORD_FORMAT",
    "EXPERIMENT_STORE_RECORD_FORMAT_VERSION",
    "EXPERIMENT_STORE_SCHEMA_ID",
    "EXPERIMENT_STORE_SCHEMA_VERSION",
    "GENERIC_EXPERIMENT_ANALYSIS_ENGINE_VERSION",
    "GENERIC_EXPERIMENT_RECORD_VERSION",
    "GENERIC_EXPERIMENT_SAFETY_DISPATCH_VERSION",
    "GENERIC_EXPERIMENT_SCHEMA_ID",
    "GENERIC_EXPERIMENT_SCHEMA_VERSION",
    "MINIMUM_CLASSIFICATION_NIGHTS_PER_ARM",
    "MINIMUM_PROSPECTIVE_NIGHTS_PER_ARM",
    "MINIMUM_PROSPECTIVE_USABLE_DURATION_MS",
    "PROSPECTIVE_SAFETY_ENGINE_VERSION",
    "PROSPECTIVE_SAFETY_POLICY_ID",
    "PROSPECTIVE_SAFETY_POLICY_VERSION",
    "PROSPECTIVE_LIFECYCLE_ENGINE_VERSION",
    "OUTCOME_CLASSIFICATION_ENGINE_VERSION",
    "OUTCOME_CLASSIFICATION_RECORD_VERSION",
    "OUTCOME_CLASSIFICATION_RULE_SET_ID",
    "OUTCOME_CLASSIFICATION_RULE_SET_VERSION",
    "OUTCOME_THRESHOLDS",
    "PS_MIN_RETROSPECTIVE_FIXTURE_ENGINE_VERSION",
    "PS_MIN_RETROSPECTIVE_FIXTURE_ID",
    "PS_MIN_RETROSPECTIVE_FIXTURE_RECORD_VERSION",
    "PS_MIN_RETROSPECTIVE_FIXTURE_VERSION",
    "AllocationInterval",
    "ArmConfounderSummary",
    "ConfounderKind",
    "ConfounderEvidence",
    "ConfounderEvidenceStatus",
    "ConfounderReportStatus",
    "EvaluationIssuedPayload",
    "EvaluationSupersededPayload",
    "AiProvenancePayload",
    "ExperimentActionPayload",
    "ExperimentAllocationError",
    "ExperimentDecisionPayload",
    "ExperimentEvent",
    "ExperimentEventPayload",
    "ExperimentEventType",
    "ExperimentEvidenceInterval",
    "ExperimentModelError",
    "ExperimentNightAllocation",
    "ExperimentNotFoundError",
    "ExperimentPeriod",
    "ExperimentProposal",
    "ExperimentProposedPayload",
    "ExperimentRecord",
    "ExperimentRevisedPayload",
    "ExperimentSetting",
    "ExperimentSettingChange",
    "ExperimentStore",
    "ExperimentStoreError",
    "GenericExperimentAnalysis",
    "GenericExperimentAnalysisStatus",
    "GenericExperimentChange",
    "GenericExperimentComparison",
    "GenericExperimentDefinition",
    "GenericExperimentError",
    "GenericExperimentMetricObservation",
    "GenericExperimentMetricSelection",
    "GenericExperimentMetricStatus",
    "GenericExperimentMode",
    "GenericExperimentPeriod",
    "GenericExperimentPeriodRole",
    "GenericExperimentSafetyPolicy",
    "GenericExperimentSafetyResult",
    "GenericExperimentSafetyStatus",
    "GenericExperimentVariableKind",
    "HypothesisDraftedPayload",
    "NoteRecordedPayload",
    "ObservationRecordedPayload",
    "ProblemRecordedPayload",
    "PSMinRetrospectiveFixture",
    "PsMinSafetyPolicyInput",
    "ReplayedExperiment",
    "RetrospectiveFixtureError",
    "RetrospectiveFixtureEvaluation",
    "RetrospectiveFixtureEvaluationStatus",
    "RetrospectiveKnownFact",
    "RetrospectiveMissingInput",
    "RetrospectiveMissingInputId",
    "RETROSPECTIVE_PROTOCOL_RECORD_VERSION",
    "RetrospectiveCohortEvidence",
    "RetrospectiveEvidenceBatch",
    "RetrospectiveEvidenceError",
    "RetrospectiveEvidenceStatus",
    "RetrospectiveEvaluationBundle",
    "RetrospectiveEvaluationError",
    "RetrospectiveProtocolError",
    "RetrospectiveProtocolInput",
    "RetrospectiveSessionClockEvidence",
    "RetrospectiveNightEvidence",
    "RetrospectiveNightEvidenceInput",
    "RetrospectiveObservation",
    "ProspectiveNightEvidence",
    "ProspectiveSafetyEvidence",
    "ProspectiveSafetyFailureCode",
    "ProspectiveSafetyGateResult",
    "ProspectiveLifecycleError",
    "ProspectiveLifecycleState",
    "ProspectiveLifecycleStatus",
    "SettingChangeConfirmedPayload",
    "SleepJournalEntryRecordedPayload",
    "SleepJournalConfounder",
    "SleepJournalEntry",
    "SleepJournalModelError",
    "LOCAL_EXPERIMENT_DATABASE_FILENAME",
    "NightAllocation",
    "NightAllocationStatus",
    "NightlyOutcomeValue",
    "OutcomeAction",
    "OutcomeArmSummary",
    "OutcomeClassification",
    "OutcomeClassificationError",
    "OutcomeClassificationResult",
    "OutcomeDirection",
    "OutcomeEvidence",
    "OutcomeId",
    "OutcomeState",
    "OutcomeThreshold",
    "SubjectiveDomainState",
    "allocate_experiment_nights",
    "analyze_generic_experiment",
    "append_experiment_event",
    "build_boundary_correction_events",
    "build_retrospective_user_evidence",
    "build_retrospective_protocol_events",
    "evaluate_outcome_classification",
    "evaluate_prospective_safety",
    "dispatch_generic_experiment_safety",
    "append_prospective_lifecycle_event",
    "replay_prospective_lifecycle",
    "evaluate_retrospective_experiment",
    "reconstruct_ps_min_experiment_fixture",
    "record_retrospective_protocol",
    "record_retrospective_user_evidence",
    "validate_experiment_history",
]
